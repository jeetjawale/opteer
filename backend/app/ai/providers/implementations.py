...
class OpenAIProvider(LLMProvider):
    ...
    async def fetch_available_models(self, api_key: str, base_url: str | None = None) -> List[Dict[str, str]]:
...
class AnthropicProvider(LLMProvider):
    ...
    async def fetch_available_models(self, api_key: str, base_url: str | None = None) -> List[Dict[str, str]]:
        url = f"{base_url.rstrip('/')}/models" if base_url else "https://api.anthropic.com/v1/models"
        async with httpx.AsyncClient() as client:
            resp = await client.get(url, headers={"x-api-key": api_key, "anthropic-version": "2023-06-01"}, timeout=10.0)
            resp.raise_for_status()
            data = resp.json()
            models = []
            for m in data.get("data", []):
                if m.get("type") == "model":
                    models.append({"id": m["id"], "name": m.get("display_name", m["id"])})
            return sorted(models, key=lambda x: x["name"])
...
class DeepSeekProvider(OpenAIProvider):
    ...
    def get_client(self, api_key: str, model: str, base_url: str | None = None) -> Any:
        return ChatOpenAI(model=model, api_key=api_key, base_url="https://api.deepseek.com/v1")

    async def fetch_available_models(self, api_key: str, base_url: str | None = None) -> List[Dict[str, str]]:
        return await super().fetch_available_models(api_key, base_url="https://api.deepseek.com/v1")
