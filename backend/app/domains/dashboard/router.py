...
async def get_dashboard_overview(
    ...
    async with log_duration("GET_DASHBOARD_OVERVIEW"):
        try:
            # 1. Fetch Stats
            stats_query = select(Application.status, Application.fit_score).where(Application.user_id == current_user.id)
            stats_result = await session.execute(stats_query)
            stats_data = stats_result.all()
            
            # 2. Fetch Top Recommendations
            recs_query = select(Application).options(joinedload(Application.job)).where(
                Application.user_id == current_user.id,
                Application.status == "saved"
            ).order_by(Application.fit_score.desc().nullslast()).limit(3)
            ...
            activity_query = select(Application).options(joinedload(Application.job)).where(
                Application.user_id == current_user.id
            ...
            for act in activity_data:
                ...
                recent_activity.append(RecentActivityItem(
                    ...
                    timestamp=act.created_at.isoformat() if act.created_at else datetime.now(timezone.utc).isoformat(),
                ))
