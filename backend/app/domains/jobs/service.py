...
class JobService:
    ...
    async def _scrape_job_posting(self, url: str, firecrawl_key: str | None) -> str:
        ...
        if firecrawl_key:
            try:
                ...
                md = scrape_result.markdown if hasattr(scrape_result, "markdown") else scrape_result.get("markdown", "")
                if md and len(md.strip()) >= 100 and not ("uses cookies" in md.lower() and len(md) < 1500):
                    scraped_jd = md
        ...
        if not scraped_jd:
            try:
                ...
                def is_safe_domain(hostname: str | None, base_domain: str) -> bool:
                    ...
                    return hostname.lower() == base_domain or hostname.lower().endswith(f".{base_domain}")
                ...
                async with httpx.AsyncClient(follow_redirects=False, headers=headers, timeout=10.0) as client:
                    ...
                    if is_safe_domain(parsed_url.hostname, "myworkdayjobs.com"):
                        ...
                        api_url = await asyncio.to_thread(validate_public_http_url, f"{parsed_url.scheme}://{parsed_url.netloc}/wday/cxs/{tenant}{parsed_url.path}")
                        api_resp = await client.get(api_url)
                        if api_resp.status_code == 200:
                            raw_html = api_resp.json().get("jobPostingInfo", {}).get("jobDescription", "")
                            if raw_html:
                    ...
                    if direct_scraped and len(direct_scraped) > 500 and not ("uses cookies" in direct_scraped.lower() and len(direct_scraped) < 1500) and "enable javascript" not in direct_scraped.lower() and not ("{{" in direct_scraped and "}}" in direct_scraped):
                        scraped_jd = direct_scraped
            except Exception as e:
                logger.warning("Fast path extraction failed: %s", str(e))
                
        if not scraped_jd:
            raise HTTPException(status_code=422, detail="Failed to extract job description from the provided URL. Please try pasting the text manually.")
        return scraped_jd

    async def _extract_job_details(self, llm, url: str, scraped_jd: str) -> tuple[str|None, str|None, str|None, str|None, str]:
        from urllib.parse import urlparse
        ...
        try:
            ...
            null_set = {"n/a", "unknown", "none", "null", "not found", "not specified", "not provided", "unspecified"}
            if company_name.lower() in null_set: company_name = None
            if role_name.lower() in (null_set | {"job description"}): role_name = None
            if location and location.lower() in null_set: location = None
            if work_model and work_model.lower() in null_set: work_model = None
        except Exception as e:
        ...
        if not company_name:
            ...
            if at_match:
                ...
                for city in [" Mumbai", " Bangalore", " Bengaluru", " Pune", " Hyderabad", " Chennai", " Delhi", " Noida", " Gurgaon"]:
                    if company_name.endswith(city): company_name = company_name[:-len(city)]
            else:
        ...
        if not role_name:
            ...
            for part in path_parts:
                if part.isdigit() or len(part) < 5: continue
                if part.count("-") >= 2:
    ...
    async def _research_company(self, llm, company_name: str, tavily_key: str | None) -> str:
        try:
            def _search_tavily():
    ...
    async def import_job(
        ...
        async with log_duration("IMPORT_URL_VALIDATION"):
            ...
            if user_configs.integration_keys:
                integration_keys = user_configs.integration_keys if isinstance(user_configs.integration_keys, dict) else {}
                from app.core.encryption import decrypt_api_key
                enc_firecrawl = integration_keys.get("firecrawl")
                if isinstance(enc_firecrawl, str): firecrawl_key = decrypt_api_key(enc_firecrawl)
                enc_tavily = integration_keys.get("tavily")
                if isinstance(enc_tavily, str): tavily_key = decrypt_api_key(enc_tavily)
            ...
            user_settings = {}
            if user_configs:
                task_models = user_configs.task_models if isinstance(user_configs.task_models, dict) else {}
                user_settings = {
            ...
            if not effective_api_key and user_configs and user_configs.llm_keys:
                llm_keys = user_configs.llm_keys if isinstance(user_configs.llm_keys, dict) else {}
            ...
            company_name, role_name, location, work_model, scraped_jd = await self._extract_job_details(llm, url, scraped_jd)
            company_research = await self._research_company(llm, company_name or "", tavily_key)

            effective_logo_url = payload.logo_url
            
            is_quality_gated, quality_gate_reason = False, None
            if payload.auto_analyze:
                is_quality_gated, quality_gate_reason = await self._run_quality_gate(llm, scraped_jd)

            existing_job = (await self.job_repo.session.execute(select(Job).where(Job.url == url))).scalar_one_or_none()
            if existing_job:
                job_id = existing_job.id
                if effective_logo_url and not existing_job.logo_url:
                    await self.job_repo.update(existing_job, logo_url=effective_logo_url)
            else:
                new_job = await self.job_repo.create(
                    url=url, company=company_name, role=role_name, location=location,
                    work_model=work_model, scraped_jd=scraped_jd, company_research=company_research,
                    logo_url=effective_logo_url
                )
                job_id = new_job.id

            from app.db.models.application import Application
            existing_app = (await self.app_repo.session.execute(select(Application).where(Application.user_id == user_id, Application.job_id == job_id))).scalar_one_or_none()

            if existing_app:
                final_analysis_status = existing_app.analysis_status or "idle"
                update_kwargs = {}
                if resume_text and existing_app.resume_text != resume_text: update_kwargs["resume_text"] = resume_text
                if resume_file_name and existing_app.resume_file_name != resume_file_name: update_kwargs["resume_file_name"] = resume_file_name
                ...
                if update_kwargs: await self.app_repo.update(existing_app, **update_kwargs)
                return JobImportResponse(
                    application_id=existing_app.id, job_id=job_id, company=company_name, status=existing_app.status,
                    analysis_status=final_analysis_status,  # type: ignore[arg-type]
                    analysis_error=existing_app.analysis_error if final_analysis_status != "queued" else None,
                    auto_analyze=bool(payload.auto_analyze) if final_analysis_status == "queued" else False,
                )

            new_app = await self.app_repo.create(
                user_id=user_id, job_id=job_id, resume_text=resume_text,
                resume_file_name=resume_file_name,
                status="rejected" if is_quality_gated else "saved",
                analysis_status="idle" if is_quality_gated else ("queued" if payload.auto_analyze else "idle"),
                analysis_error=None, is_quality_gated=is_quality_gated, quality_gate_reason=quality_gate_reason,
            )
            return JobImportResponse(
                application_id=new_app.id, job_id=job_id, company=company_name, status=new_app.status,
                analysis_status="idle" if is_quality_gated else ("queued" if payload.auto_analyze else "idle"),  # type: ignore[arg-type]
                analysis_error=None, auto_analyze=bool(payload.auto_analyze) if not is_quality_gated else False,
            )
