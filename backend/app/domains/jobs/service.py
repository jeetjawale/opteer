...
def validate_public_http_url(url: str) -> str:
    ...
    if not parsed.hostname:
        raise HTTPException(status_code=400, detail="Job URL must include a valid hostname.")
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="Job URL host could not be resolved.")
    for ip in ips_to_check:
        if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_multicast or ip.is_reserved or ip.is_unspecified:
            raise HTTPException(status_code=400, detail="Job URL host resolves to a disallowed IP address.")
    return url
...
class JobService:
    ...
    async def get_jobs(self, search: Optional[str] = None, company: Optional[str] = None, sort_by: str = "created_at", sort_order: str = "desc", page: int = 1, per_page: int = 50):
        offset = (page - 1) * per_page
        query = select(Job)
        if search:
            query = query.where(or_(Job.role.ilike(f"%{search}%"), Job.company.ilike(f"%{search}%")))
        if company:
    ...
    async def import_job(self, payload: ImportJobRequest, user_id: str | UUID, x_user_api_key: Optional[str] = None, background_tasks: Optional[Any] = None):
        ...
        async with log_duration("IMPORT_URL_VALIDATION"):
            ...
            try:
                ...
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to load user config: {str(e)}")
            ...
            if payload.auto_analyze:
                try:
                    ...
                    if user_configs.last_credit_reset != today:
                        ...
                        await self.user_configs_repo.update(user_configs, daily_analysis_credits=50, last_credit_reset=today)
                    ...
                    user_configs = await self.user_configs_repo.update(user_configs, daily_analysis_credits=user_configs.daily_analysis_credits - 1)
                        ...
                        async with httpx.AsyncClient(follow_redirects=False, headers=headers, timeout=10.0) as client:
                            ...
                            if is_safe_domain(parsed_url.hostname, "myworkdayjobs.com"):
                                ...
                                if api_resp.status_code == 200:
                                    ...
                                    if raw_html: direct_scraped = await asyncio.to_thread(clean_html, raw_html)
            ...
            if not effective_api_key:
                if user_configs and user_configs.active_llm_provider and user_configs.llm_keys:
            ...
            if not effective_api_key:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No LLM API key configured. Please go to Settings and add an API key.")
            ...
            try:
                ...
                if company_name.lower() in {"n/a", "unknown", "none", "null", "not found", "not specified", "not provided", "unspecified"}:
                    company_name = None
                if role_name.lower() in {"n/a", "unknown", "none", "null", "not found", "job description"}:
                    role_name = None
                if location and location.lower() in {"n/a", "unknown", "none", "null", "not found"}:
                    location = None
                if work_model and work_model.lower() in {"n/a", "unknown", "none", "null", "not found"}:
                    work_model = None
            except Exception as e:
            ...
            if not company_name:
                ...
                if at_match:
                    ...
                    common_cities = [' Mumbai', ' Bangalore', ' Bengaluru', ' Pune', ' Hyderabad', ' Chennai', ' Delhi', ' Noida', ' Gurgaon']
                ...
                try:
                    ...
                    if match:
                        ...
                        if str(qg_json.get("passes_gate")).lower() == "false":
                            ...
                            quality_gate_reason = qg_json.get("reason", "Failed quality gate.")
                except Exception as e:
                    print(f"Quality gate failed to run: {e}")

            try:
                existing_job_res = await self.job_repo.session.execute(select(Job).where(Job.url == url))
