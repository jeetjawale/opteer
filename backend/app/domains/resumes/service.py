...
class ResumeService:
    ...
    async def list_resumes(self, user_id: str | uuid.UUID, page: int = 1, per_page: int = 50) -> Dict[str, Any]:
        ...
        # Get paginated data
        query = select(Resume).where(Resume.user_id == user_id).order_by(Resume.created_at.desc()).offset(offset).limit(per_page)
    ...
    async def upload_resume_file(self, user_id: str | uuid.UUID, file: UploadFile) -> Dict[str, Any]:
        if isinstance(user_id, str):
            user_id = uuid.UUID(user_id)
    ...
    async def create_resume(self, user_id: str | uuid.UUID, payload_data: Dict[str, Any]) -> Dict[str, Any]:
        if isinstance(user_id, str):
            user_id = uuid.UUID(user_id)
    ...
    async def get_resume(self, user_id: str | uuid.UUID, resume_id: str | uuid.UUID) -> Optional[Dict[str, Any]]:
        if isinstance(user_id, str): user_id = uuid.UUID(user_id)
        if isinstance(resume_id, str): resume_id = uuid.UUID(resume_id)
    ...
    async def update_resume(self, user_id: str | uuid.UUID, resume_id: str | uuid.UUID, update_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if isinstance(user_id, str): user_id = uuid.UUID(user_id)
        if isinstance(resume_id, str): resume_id = uuid.UUID(resume_id)
    ...
    async def delete_resume(self, user_id: str | uuid.UUID, resume_id: str | uuid.UUID) -> None:
        if isinstance(user_id, str): user_id = uuid.UUID(user_id)
        if isinstance(resume_id, str): resume_id = uuid.UUID(resume_id)
        ...
        if resume.file_url:
            if not resume.file_url.startswith("http"):
                ...
                if resume.file_url.startswith(f"{settings.API_URL}/api/storage/resumes/"):
                    path = resume.file_url.replace(f"{settings.API_URL}/api/storage/resumes/", "")
                    await self.storage.delete(bucket="resumes", path=path)
            
        await self.resume_repo.delete(resume_id)

    async def delete_resume_file(self, user_id: str | uuid.UUID, resume_id: str | uuid.UUID) -> Optional[Dict[str, Any]]:
        if isinstance(user_id, str): user_id = uuid.UUID(user_id)
        if isinstance(resume_id, str): resume_id = uuid.UUID(resume_id)
