    ...
    async with log_duration("UPDATE_SETTINGS"):
        try:
            ...
            if "llm_keys" in update_data and update_data["llm_keys"]:
                ...
                existing_keys = (
                    user_config.llm_keys if user_config and user_config.llm_keys else {}
                )
                ...
                for provider, data in llm_keys_payload.items():
                    ...
                    if "api_key_encrypted" in data and data["api_key_encrypted"]:
                        ...
                        if "••••" not in raw_key:
                            existing_keys[provider]["api_key_encrypted"] = (
            ...
            if "integration_keys" in update_data and update_data["integration_keys"]:
                ...
                for provider, api_key in integration_keys_payload.items():
                    if api_key:
                        if "••••" not in api_key:
                            existing_integration_keys[provider] = encrypt_api_key(
                                api_key
                            )
                    else:
...
async def validate_llm_key(
    payload: LLMValidateRequest, current_user=Depends(get_current_user)
):
    """
...
async def get_llm_models(
    ...
    async with log_duration("FETCH_MODELS"):
        ...
        if not api_key and provider != "ollama":
            raise HTTPException(
                status_code=400, detail="API key not found for provider."
            )
...
async def validate_integration_key(
    payload: IntegrationValidateRequest, current_user=Depends(get_current_user)
):
    """
