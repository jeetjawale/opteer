...
class FitScoreResult(BaseModel):
    fit_score: Optional[int] = Field(
        None, ge=0, le=100, description="Fit score from 0 to 100"
    )
    matched_skills: List[str] = Field(
        ..., description="List of user skills matching the job description"
    )
    missing_skills: List[str] = Field(
        ...,
        description="List of critical skills/requirements missing from user profile",
    )
    key_requirements: List[str] = Field(
        ..., description="List of primary job requirements identified"
    )
    summary: str = Field(
        ..., description="An explicit Markdown summary explaining the fit assessment."
    )
...
class InterviewQuestion(BaseModel):
    question: str = Field(
        ...,
        description="Potential interview question based on the role and resume gaps",
    )
    suggested_answer: str = Field(
        ...,
        description="Tailored guide/points on how the user should answer this question",
    )


class InterviewPrepResult(BaseModel):
    questions: List[InterviewQuestion] = Field(
        ..., description="List of custom-generated interview prep questions"
    )
...
class ResumeEdit(BaseModel):
    ...
    reasoning: str = Field(
        ...,
        description="Why this edit will improve the match against the job description",
    )
...
class JobBase(BaseModel):
    ...
    scraped_jd: Optional[str] = Field(
        None, description="Scraped raw job description text"
    )
...
class JobImportResponse(BaseModel):
    ...
    company: Optional[str] = Field(
        None, description="The company name extracted from the job posting"
    )
    status: str = Field(..., description="Status of the application (e.g. 'saved')")
    analysis_status: AnalysisStatus = Field(
        AnalysisStatus.IDLE, description="Durable AI analysis state"
    )
    analysis_error: Optional[str] = Field(
        None, description="Last analysis error, when analysis failed"
    )
    auto_analyze: bool = Field(
        False, description="Whether auto-analysis was requested for this import"
    )
...
class ApplicationResponse(ApplicationBase):
    ...
    summary: Optional[str] = None
    cover_letter: Optional[str] = None
    interview_prep: Optional[Any] = (
        None  # Stored as jsonb in DB (matches InterviewPrepResult structure)
    )
...
class JobAnalysisRequest(BaseModel):
    ...
    resume_text: Optional[str] = Field(
        None, description="The plain text of the user's resume"
    )
    resume_file_url: Optional[str] = Field(
        None, description="Optional pre-uploaded resume file URL"
    )
    resume_file_name: Optional[str] = Field(
        None, description="Optional resume file name"
    )
...
class ResumeBase(BaseModel):
    name: str = Field(
        ..., min_length=1, description="User-visible label for the resume"
    )
    content: str = Field(
        ..., min_length=50, description="Full parsed resume text content"
    )
...
class UserConfigResponse(BaseModel):
    ...
    active_models: Dict[str, str] = Field(
        default_factory=dict, description="Map of provider -> active model ID"
    )
    base_urls: Dict[str, str] = Field(
        default_factory=dict, description="Map of provider -> custom base URL"
    )
    task_models: Dict[str, str] = Field(
        default_factory=dict, description="Map of task -> model ID"
    )
    integration_providers_configured: Dict[str, bool] = Field(
        default_factory=dict,
        description="Map of integration -> true if key is configured",
    )
